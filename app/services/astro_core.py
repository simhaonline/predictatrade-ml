...
class AstroCore:
    ...
    # ----------------------------------------------------------
    # Saturn retrograde check
    # ----------------------------------------------------------
    def is_saturn_retrograde(self, dt: datetime) -> bool:
        """Return True if Saturn is retrograde at the given datetime."""
        positions = self.get_sidereal_positions(dt)
        saturn = positions.get("Saturn")
        if saturn is None:
            return False
        return bool(saturn.get("retrograde", False))

    # ----------------------------------------------------------
    # Nakshatra lookup by longitude (used by DashaService)
    # ----------------------------------------------------------
    def _nakshatra_from_long(self, lon: float) -> Dict[str, Any]:
        """
        Return nakshatra info for a given sidereal longitude.
        Used by DashaService for Vimshottari dasha timeline.
        """
        norm = lon % 360.0
        n_index = int(norm // NAKSHATRA_SPAN)
        nak_name = NAKSHATRAS[n_index]
        within_nak = norm - n_index * NAKSHATRA_SPAN
        pada_index = int(within_nak // PADA_SPAN)
        pada = pada_index + 1
        ...
        return {
            ...
            "name": nak_name,
            "pada": pada,
            "longitude_in_nakshatra": round(within_nak, 4),
        }
